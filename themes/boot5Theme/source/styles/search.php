...
$resource_types = get_terms(array('taxonomy' => 'resource_type', 'fields' => 'id', 'hide_empty' => true));

$filter_type = isset($_GET['filter_type']) ? $_GET['filter_type'] : array();
get_header(); 
?>
...
<div class="container">
	...
	<?php if ( have_posts() ) : ?>
		...
		<?php while ( have_posts() ) : the_post(); 
			...
			if (($page_template == 'page-templates/page-asset-page.php') || ($post_type == 'resource')) {
				// guide section or asset
				$resource_type = get_the_terms($post, 'resource_type');
				$resource_label['icon'] = (is_array($resource_type)) ? get_field('icon', $resource_type[0]) : 'las la-file-alt';
				$resource_label['name'] = (is_array($resource_type)) ? $resource_type[0]->name : 'Educational Article';
			} else if ($page_template == 'page-templates/page-guide-overview.php') {
				$resource_label['icon'] = 'las la-book';
				$resource_label['name']= 'Learning Guide';
				$post_class = "learning-guide";
			} else if ($post_type == 'post') {
				$resource_label['icon'] = 'las la-comment';
				$resource_label['name'] = 'News &amp; Insights';
			} else if ($post_type == 'product') {
				$resource_label['icon'] = 'las la-newspaper';
				$resource_label['name'] = 'Publication';
			} else {
					...
					<div class="article-inner">
						...
						<header class="entry-header">
							...
							<?php
								...
								echo ($resource_label) ? '<div class="resource-title"><i class="' . $resource_label['icon'] . '"></i><span>' . $resource_label['name'] . '</span></div>' : '';
